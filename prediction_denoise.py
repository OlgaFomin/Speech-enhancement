...
def prediction_n2n(weights_path, name_model, audio_dir_prediction, dir_save_prediction, audio_input_prediction,
audio_output_n2n_prediction, sample_rate, min_duration, frame_length, hop_length_frame, n_fft, hop_length_fft):
    ...
    # load weights into new model
    #loaded_model.load_weights(weights_path+'/'+name_model+'.h5')
    loaded_model.load_weights(weights_path+'/model_best_n2n.h5')
    print("Loaded model from disk")
    ...
    #Save all frames in one file
    denoise_long = audio_denoise_recons.reshape(1, nb_samples * frame_length)*10
    librosa.output.write_wav(dir_save_prediction + audio_output_n2n_prediction, denoise_long[0, :], sample_rate)
